...
## 💻 1. Fraud Analyst: Queue Triage Workflow
...
### Processing Alerts
* Hover over any alert card to view the specific **Triggering Signals** emitted by the platform (e.g. Device Anomaly, IP Location Spoofing).
* Read the **Trigger Badge** (e.g. `Device Custom Trigger (>=70)`) to understand *why* the card was flagged, especially if its composite score is below the Master threshold.
* Click one of the triage buttons at the bottom of the card:
  * **Block**: Flag transaction as fraudulent and prevent payout.
...
## 📈 2. Risk Manager: Threshold Simulation Workflow
